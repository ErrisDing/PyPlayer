#!/usr/bin/env python3
"""
OGG Vorbis metadata extractor with multi-strategy cover art extraction
"""
...
class OGGExtractor(MetadataExtractor):
    ...
    def extract(cls, filepath: str) -> ExtractionResult:
        ...
        try:
            ...
            result.success = False
            result.errors.append(f"Failed to extract OGG metadata: {e}")
    ...
    def _extract_pictures(cls, audio, result: ExtractionResult) -> None:
        ...
        Strategy 1: Vorbis comment METADATA_BLOCK_PICTURE (base64 encoded)
        Strategy 2: Legacy COVERART tag (base64 encoded)
        """
        # Strategy 1: METADATA_BLOCK_PICTURE
        ...
        # Strategy 2: Legacy COVERART tag
    ...
    def _extract_metadata_block_picture(cls, audio, result: ExtractionResult) -> None:
        ...
        if isinstance(encoded_data, list):
            for item in encoded_data:
                cls._decode_and_add_picture(item, result)
        else:
            cls._decode_and_add_picture(encoded_data, result)

    @classmethod
    def _decode_and_add_picture(cls, encoded_data: str, result: ExtractionResult) -> None:
        """Decode base64 picture data and add to result"""
        from mutagen.flac import Picture
